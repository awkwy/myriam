...
class UnifiedYFinanceCollector:
    ...
    def collect_options_data(self, ticker: str, 
        ...
        # 1. Obtenir les informations du sous-jacent
        stock_info = self._get_stock_info(ticker)
        if not stock_info:
            return pd.DataFrame()
        
        spot_price = stock_info['price']
        logging.info(f"💰 Prix spot: ${spot_price:.2f}")
    ...
    def _get_stock_info(self, ticker: str) -> Optional[Dict]:
        """Récupère les informations du sous-jacent."""
        try:
            stock = yf.Ticker(ticker)
            
            # Essayer d'abord info
            try:
                info = stock.info
            ...
            # Sinon utiliser l'historique
            hist = stock.history(period="5d")
            if hist.empty:
                logging.error(f"❌ Pas de données pour {ticker}")
                return None
            ...
            logging.error(f"❌ Erreur récupération {ticker}: {e}")
            return None
...
def main():
    ...
    df_today = collector.collect_options_data(
        ticker=TICKER,
        max_expirations=50,  # Limiter pour la démo
        include_greeks=True
    )
    ...
    if not df_today.empty:
        ...
        print(f"\n📅 Collecte historique sur 3 jours")
        print("-" * 50)
